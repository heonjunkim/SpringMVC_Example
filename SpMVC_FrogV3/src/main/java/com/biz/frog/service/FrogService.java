...
public class FrogService {
	...
	private String frogFile ; 
	
	public FrogService() {
		this.frogFile = "frog.txt";
		// this.serverRootPath = context.getRealPath("/");
	}
	
	public List<FrogVO> selectAll() {
		...
		try {
			...
			File file = new File(this.serverRootPath,frogFile);
			
			path = Paths.get(file.toString());
			List<String> strList = Files.readAllLines(path);
			List<FrogVO> frogList = new ArrayList<FrogVO>();
			
			// title, content, user
			for(String str : strList) {
				String[] sSplit = str.split(":");
				FrogVO frogVO = new FrogVO();
				frogVO.setTitle(sSplit[0]);
				frogVO.setContent(sSplit[1]);
				frogVO.setUser(sSplit[2]);
				frogList.add(frogVO);
			}
			return frogList;
	...
	public void insert(FrogVO frogVO) {
